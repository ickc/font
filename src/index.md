...
- [Diagrams in the same face as the prose](/mermaid.html)
...
A fenced `mermaid` block is a picture in all eight outputs, and
`config/mermaid.lua` is what makes it one. It is loaded by all four recipes
rather than the two the PDF filter needs, because a diagram is content: HTML
gets the SVG inlined into the page, so its labels resolve against the same
`@font-face` rules the body text does; Typst gets the same SVG as a file; and
LuaLaTeX, which cannot read SVG without `--shell-escape` and Inkscape, gets a
PDF derived from it.

Quarto's own mermaid support is not used, and could not be. A native diagram is
an executable cell, which Quarto accepts only in a `.qmd`, and every source here
is a `.md` because the Makefile reads the same file. It would also have covered
only Quarto's half of the matrix.

Only the SVG is checked in, and a browser is what decides that. Drawing one is
what needs Chrome and an npm fetch, so it is generated by `pixi run
render-diagrams` and committed, for the reason `bin/script-ranges.lua` is:
...
[`src/mermaid.md`](/mermaid.html) is that page, and explains the rest of it.
...
`pixi run render-diagrams` run once to draw it.
